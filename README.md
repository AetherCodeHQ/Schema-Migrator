...
> Schema-Migrator - Open source tool by AetherCodeHQ

`cli` `database` `golang` `migration` `schema`

## What is it?

**Schema Migrator** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Schema-Migrator.git
cd Schema-Migrator
go build -o Schema-Migrator .
```

### Run
```bash
./Schema-Migrator --help
```
...
```bash
# Basic usage
./Schema-Migrator --path ./target

# With options
./Schema-Migrator --path ./target --format json --output report.json

# Verbose mode
./Schema-Migrator --path ./target --verbose
```
...
### Basic scan
```bash
./Schema-Migrator --path ./my-project
```

### JSON report
```bash
./Schema-Migrator --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Schema Migrator
  run: ./Schema-Migrator --path . --format json --output report.json
```
